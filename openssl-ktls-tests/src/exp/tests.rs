...
use crate::exp::ffi::{BIO_f_base64, BIO_flush, BIO_get_mem_ptr, BIO_push, BUF_MEM};
...
mod network {
    ...
    use crate::exp::ffi::{BIO_do_connect, BIO_f_null, BIO_free, BIO_new_connect, BIO_push};
    use openssl_ktls::{
        SslStream,
        ffi::{BIO_NOCLOSE, SSL_OP_ENABLE_KTLS},
    };
    ...
    fn test_bio_https_request() {
        ...
        let send_enabled = unsafe { openssl_ktls::ffi::BIO_get_ktls_send(wbio) };
        let recv_enabled = unsafe { openssl_ktls::ffi::BIO_get_ktls_recv(rbio) };
        ...
        let https_request = b"GET /ip HTTP/1.1\r\nHost: httpbin.org\r\nConnection: close\r\n\r\n";
        let bytes_written = unsafe {
    ...
    fn test_bio_https_with_ktls() {
        ...
        let sni_hostname = c"httpbin.org".as_ptr() as *mut c_char;
        unsafe {
            openssl_sys::SSL_set_tlsext_host_name(bio_socket_stream.ssl().as_ptr(), sni_hostname)
        };
        ...
        if send_enabled > 0 || recv_enabled > 0 {
            ...
            println!("ℹ️  KTLS not auto-enabled (this is the expected behavior we investigated)");
        ...
        println!("Test completed - demonstrated HTTPS with KTLS option set and BIOSocketStream");
    }
