...
// A `tokio`-only build has no consumer of the shared engine yet: the Tokio
// adapter is still a documentation skeleton. The allowance is scoped to exactly
// that configuration, so the compio and default builds remain strict, and it is
